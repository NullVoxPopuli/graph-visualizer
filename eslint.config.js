...
export default [
  ...configs.ember(import.meta.dirname),
  {
    ignores: [
      ...
      "node_modules",
      "public",
      // wasm-pack output (regenerated by `pnpm build:wasm`); ships its
      // own eslint-disable header but there's no value in linting it.
      "app/lib/wasm/",
      "crates/",
    ],
  },
  {
    // Benchmark harness: Node scripts that walk typed-array fixtures the
    // same way the layout hot paths do, so the same `array[i]!` rationale
    // as `app/lib/**` applies. Top-level `await` + `process`/`performance`.
    files: ["bench/**/*.ts"],
    languageOptions: {
      globals: { process: "readonly", performance: "readonly", console: "readonly" },
    },
    rules: {
